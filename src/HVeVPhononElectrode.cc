// I have implemented a custom electrode class inherited from G4CMPPhononElectrode so that I can get a better understanding of what is going on inside it. - Miao Yu, 09/18/2025.
// It can be removed once everything is confirmed to be working properly.
...
HVeVPhononElectrode::HVeVPhononElectrode()
...
G4bool HVeVPhononElectrode::IsNearElectrode(const G4Step& /*step*/) const {
    //G4cout << "Running IsNearElectrode from HVeVPhononElectrode" << G4endl; // I have checked this function is called
    //G4double rand = G4UniformRand();
    //G4double filmAbsorption = GetMaterialProperty("filmAbsorption");
    //G4bool isNear = rand < filmAbsorption;
    //G4cout << rand << " " << filmAbsorption << " " << isNear << G4endl;
    //return isNear;
...
AbsorbAtElectrode(const G4Track& track, const G4Step& step,
  ...
  if (!kaplanQP) {
    ...
    kaplanQP = new HVeVKaplanQP(theSurfaceTable, verboseLevel);
    kaplanQP->SetVerboseLevel( verboseLevel);
  }
