...
void plotThicknessScan_uncoupled() {

    const int Nfiles = 5;
    double thickness[Nfiles] = {10, 20, 35, 50, 50};
    double frac[Nfiles];
    double err[Nfiles];

    // Uncoupled scan files
    const char* filenames[Nfiles] = {
        "10nm_uncoupled_merged.root",
        "20nm_uncoupled_merged.root",
        "35nm_uncoupled_merged.root",
        "50nm_uncoupled_merged.root",
        "50nm_z-1.0_uncoupled_merged.root"
    };
    ...
    gSim->Draw("APL");
    c->SaveAs("ThicknessScan_Uncoupled_EnergyFraction.png");

    std::cout << "\nPlot saved to ThicknessScan_Uncoupled_EnergyFraction.png\n";
