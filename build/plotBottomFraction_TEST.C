...
void plotBottomFraction_TEST(){

    const int Nfiles = 5;
    double thickness[Nfiles] = {5, 20, 40, 80, 150};
    double frac[Nfiles];

    const char* filenames[Nfiles] = {
        "5nm_0VbfilmTEST.root",
    ...
    g->SetTitle("Bottom Film Phonon Absorption (TEST);Film Thickness (nm);<N_{bottom}/N_{total}>");
    g->SetLineWidth(4);
    g->SetMarkerStyle(20);
    g->SetMarkerSize(1.8);
    ...
    g->Draw("APL");
    c->SaveAs("BottomPhononFraction_vs_Thickness_TEST.png");
}
